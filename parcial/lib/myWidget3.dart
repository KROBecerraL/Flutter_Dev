...
class FavoritoListView extends StatefulWidget {
  const FavoritoListView({Key? key}) : super(key: key);

  @override
  _FavoritoListViewState createState() => _FavoritoListViewState();
}

class _FavoritoListViewState extends State<FavoritoListView> {
  ...
  void initState() {
    ...
    _articulosFuture = Future.value([]);
    _loggedUser().then((id) {
      _articulosFuture = userDatabase.getArticulosFavoritos(int.parse(id));
    });
    _starredMap = {};
  }
  ...
  Future<String> _loggedUser() async {
    final userDatabase =
        Provider.of<AppState>(context, listen: false).userDatabase;
    final idOfUser = await JWTGenerator().authenticateToken();
    return idOfUser;
                    ...
                    child: ListTile(
                      ...
                      trailing: IconButton(
                        ...
                        onPressed: () async {
                          String idOfUser = '';
                          try {
                            idOfUser = await JWTGenerator().authenticateToken();
                          } catch (e) {
                          ...
                          if (!isStarred) {
                            ...
                            await userDatabase.addArticuloToFavoritos(
                                snapshot.data![index].id, int.parse(idOfUser));
                            print("Añadido");
                          } else {
