...
Future<void> _checkLogin(BuildContext context) async {
  ...
  if (token != '') {
    final url = Uri.parse('http://10.0.2.2:8080/protected');
    ...
    if (response.statusCode == 200) {
      ...
      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => VerifiedPage(title: 'Verified Page'),
        ),
      );
    }
  }
...
class VerifiedPage extends StatelessWidget {
  const VerifiedPage({Key? key, required this.title}) : super(key: key);
